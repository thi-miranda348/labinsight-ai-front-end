...
export function InteractiveAnalysisView({ report, patient }: viewProps) {
  ...
  const abnormalResults = report.results.filter(
    (r) => r.status === "Critical" || r.status === "Borderline",
  );
            ...
            <div>
              ...
              <div className="w-full grid grid-cols-1 md:grid-cols-2 items-center justify-between gap-3">
                <div className="w-full h-full bg-background rounded-lg px-4 py-4 lg:px-6 lg:py-6 border border-border shadow-sm">
                  ...
                  {primaryConcern ? (
                    <div
                      className={`flex items-center justify-start gap-2 ${primaryConcern.status === "Critical" ? "text-destructive" : "text-amber-600"}`}
                    >
        ...
        <div className="bg-background border rounded-xl shadow-sm overflow-hidden sticky top-20 flex flex-col h-[600px] lg:h-[calc(70vh)]">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs md:text-sm">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`space-y-1.5 ${msg.sender === "doctor" ? "flex flex-col items-end" : ""}`}
              >
                <span
                  className={`text-[10px] lg:text-xs font-bold uppercase flex flex-row items-center justify-start gap-0 ${msg.sender === "doctor" ? "text-muted-foreground text-right" : "text-primary"}`}
                >
                  {msg.sender === "ai" ? (
                    <>
                ...
                <div
                  className={`p-3.5 rounded-xl leading-relaxed shadow-3xs max-w-[85%] ${
                    msg.sender === "doctor"
